...
public class ParserGenerator {
  ...
  private void generateRootParserContent() {
    ...
    out("%s %s = enter_section_(%s, 0, _COLLAPSE_, null, null);", shortMarker, N.marker, N.builder);
    ...
    out("}");
    newLine();
    out("boolean parse_root_(%s %s, %s %s, int %s) {", shortET, N.root, shortPB, N.builder, N.level);
  ...
  void generateNode(BnfRule rule, BnfExpression initialNode, String funcName, Set<BnfExpression> visited) {
    ...
    out("%sboolean %s(%s %s, int %s%s) {", !isRule ? "private " : isPrivate ? "" : "public ",
    ...
    if (sectionRequiredSimple) {
      if (!sectionMaybeDropped) {
        out("%s %s = enter_section_(%s, %s, %s);", shortMarker, N.marker, N.builder, N.level, elementTypeRef);
      }
    }
  ...
  private @NotNull NodeArgument generateWrappedNodeCall(@NotNull BnfRule rule, @Nullable BnfExpression nested, @NotNull String nextName) {
    ...
    if (nodeCall instanceof MetaMethodCall) {
      ...
      return () -> format("this::%s", methodCall.getMethodName());
    }
    else {
